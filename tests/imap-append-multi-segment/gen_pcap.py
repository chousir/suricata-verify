#!/usr/bin/env python3
"""Generate input.pcap for imap-append-multi-segment.

The APPEND literal body is split across two client->server TCP segments at
a byte offset that does NOT land on a line boundary, exercising
IMAPState.request_literal_remaining persisting the declared byte count
across separate parse_request() calls -- the request-side mirror of
imap-literal-multi-segment's response-side coverage.
"""
...
pkts.append(cli("PA", c_seq, s_seq, seg_b))
c_seq += len(seg_b)
pkts.append(srv("A", s_seq, c_seq))
